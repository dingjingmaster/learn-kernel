...
static struct buffer_head *aufs_find_entry(struct inode *dir, const struct qstr *child, struct aufs_dirent **res_dir);
    ...
    while (ctx->pos < dir->i_size) {
        ...
        do {
            ...
            if (de->ino) {
                ...
                if (!dir_emit(ctx, de->name, size, le16_to_cpu(de->ino), DT_UNKNOWN)) {
                    brelse(bh);
                    return 0;
                }
            }
...
static int aufs_create(struct user_namespace *mnt_userns, struct inode *dir, struct dentry *dentry, umode_t mode, bool excl)
{
    int err;
    struct inode *inode;
...
static int aufs_link(struct dentry *old, struct inode *dir, struct dentry *new)
{
...
static int aufs_rename(struct user_namespace *mnt_userns, struct inode *old_dir, struct dentry *old_dentry, struct inode *new_dir, struct dentry *new_dentry, unsigned int flags)
{
...
static inline int aufs_namecmp(int len, const unsigned char *name, const char *buffer)
{
...
static struct buffer_head *aufs_find_entry(struct inode *dir, const struct qstr *child, struct aufs_dirent **res_dir)
{
    ...
    *res_dir = NULL;
    ...
    while (block * aufs_BSIZE + offset < dir->i_size) {
        ...
        de = (struct aufs_dirent *)(bh->b_data + offset);
        offset += aufs_DIRENT_SIZE;
        if (le16_to_cpu(de->ino) && aufs_namecmp(namelen, name, de->name)) {
            *res_dir = de;
            return bh;
        }
